...
import { regenerateTripShareCode } from "@/src/api/tripCollaborationApi";
...
export default function TripShareCodeScreen() {
    ...
    async function handleGenerateCode() {
        ...
        try {
            ...
            const result = await regenerateTripShareCode(tripId, { defaultRole: role });
            setShareCode(result);
            Alert.alert(
                "Invite code ready",
    ...
    return (
        <AppScreen contentContainerStyle={styles.screenContent}>
            <View style={styles.header}>
                ...
                <View style={styles.headerTextGroup}>
                    ...
                    <Text style={styles.subtitle}>
                        Generate a single-use invite code. When someone successfully requests to join, the code becomes used.
                    </Text>
                </View>
            </View>
            ...
            {shareCode ? (
                <AppCard title="Current invite" subtitle="Use the code for manual entry or the link for deep-link opening." contentStyle={styles.codeCardContent}>
                    ...
                    <View style={styles.expiryBox}>
                        ...
                        <View style={styles.expiryTextGroup}>
                            <Text style={styles.expiryTitle}>Expires {formatDateTime(shareCode.expiresAt)}</Text>
                            <Text style={styles.expirySubtitle}>
                                This code is single-use. It also becomes invalid if you regenerate another code.
                            </Text>
                        </View>
                    </View>
                    ...
                    <View style={styles.linkBox}>
                        ...
                        <Text style={styles.linkText} numberOfLines={2}>{shareCode.inviteLink}</Text>
                    </View>

                    <View style={styles.actionGrid}>
                        <AppButton title="Copy Code" onPress={handleCopyCode} variant="outline" fullWidth={false} style={styles.actionButton} />
                        <AppButton title="Copy Link" onPress={handleCopyLink} variant="outline" fullWidth={false} style={styles.actionButton} />
                    </View>
                    ...
                    <AppButton
                        ...
                        loading={isSharing}
                        leftIcon={<Ionicons name="share-outline" size={19} color={colors.textLight} />}
                    />
                </AppCard>
            ) : null}
        </AppScreen>
...
const styles = StyleSheet.create({
    ...
    roleLabel: { color: colors.text, fontSize: typography.bodySmall, fontWeight: fontWeight.bold },
    roleRow: { gap: spacing.md },
    roleChip: { borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, padding: spacing.md, gap: spacing.sm },
    roleChipSelected: { borderColor: colors.primary, backgroundColor: colors.primarySoft },
    roleHelpText: { color: colors.textMuted, fontSize: typography.caption, lineHeight: 18, fontWeight: fontWeight.semibold },
    codeCardContent: { gap: spacing.lg },
    codeBox: { borderRadius: radius.lg, backgroundColor: colors.primary, padding: spacing.lg, gap: spacing.xs },
    codeLabel: { color: "#DBEAFE", fontSize: typography.caption, fontWeight: fontWeight.bold, textTransform: "uppercase", letterSpacing: 0.6 },
    codeText: { color: colors.textLight, fontSize: typography.heading, fontWeight: fontWeight.bold, letterSpacing: 1.2 },
    ...
    linkBox: { borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceSoft, padding: spacing.md, gap: spacing.xs },
    linkLabel: { color: colors.textMuted, fontSize: typography.caption, fontWeight: fontWeight.bold, textTransform: "uppercase", letterSpacing: 0.4 },
    linkText: { color: colors.text, fontSize: typography.bodySmall, lineHeight: 20, fontWeight: fontWeight.semibold },
