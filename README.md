...
On pull requests and pushes to `main`, the workflow runs the backend Maven test suite. On successful pushes to `main`, GitHub Actions triggers a Render deployment through a protected Render deploy hook secret.
...
The current frontend production API URL is:

```ts
export const API_BASE_URL = "https://wandermate-fullstack.onrender.com/The-Project";
```

This is used for testing the mobile frontend against the deployed Render backend.

For Android emulator connecting to a local IntelliJ backend on port `8080`, use:

```ts
export const API_BASE_URL = "http://10.0.2.2:8080/The-Project";
```

For Android emulator connecting to the Docker backend on port `8082`, use:
